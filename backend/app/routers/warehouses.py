...
def list_warehouses(user: dict = Depends(require_staff)):
    result = supabase.table("warehouses").select("*").eq("company_id", user["company_id"]).execute()
    return result.data or []


@router.post("/", response_model=WarehouseOut)
def create_warehouse(data: WarehouseCreate, user: dict = Depends(require_admin)):
...
def update_warehouse(warehouse_id: str, data: WarehouseUpdate, user: dict = Depends(require_admin)):
...
def delete_warehouse(warehouse_id: str, user: dict = Depends(require_admin)):
